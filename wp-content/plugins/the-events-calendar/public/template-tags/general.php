	...
	function tribe_events_event_schedule_details( $event = null, $before = '', $after = '' ) {
		...
		if ( tribe_event_is_multiday( $event ) ) { // multi-date event
			...
			if ( tribe_event_is_all_day( $event ) ) {
				...
				$schedule .= '<span class="value-title" title="'. $microformatStartFormat .'" itemprop="startDate"content="'. date("c", strtotime($microformatStartFormat)) .'"></span>';
				$schedule .= '</span>'.$time_range_separator;
				$schedule .= '<span class="date-end dtend">';
				$schedule .= tribe_get_end_date( $event, true, $format2ndday );
				$schedule .= '<span class="value-title" title="'. $microformatEndFormat .'" itemprop="endDate"content="'. date("c", strtotime($microformatEndFormat)) .'"></span>';
			} else {
			...
			if ( tribe_get_start_date( $event, false, 'g:i A' ) === tribe_get_end_date( $event, false, 'g:i A' ) ) { // Same start/end time
				...
				$schedule .= ( $show_end_time ? tribe_get_end_date( $event, false, $time_format ) : '' ) . '<span class="value-title" title="'. $microformatEndFormat .'" itemprop="endDate"content="'. date("c", strtotime($microformatEndFormat)) .'"></span>';
			}
		}
